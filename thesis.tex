...
\begin{itemize}
\item \bf{Why - If we know how much time it takes to develop a scalable application with
...
\item \bf{How - By keeping track of spent time for writing the software, 
and also by doing it more than one time we can see how much time is saved
when you starts to get used to mpi.}
\end{itemize}
\item Lines of code
\begin{itemize}
\item \bf{Why - The easiest metric of how complex and hard to write and
 ...
 difference.}
\item \bf{How - Count number of lines with \emph{wc -l}}
\end{itemize}
\item How easy the code is to understand
\begin{itemize}
\item \bf{Why - Code that is easy to understand is also easy to maintain and extend. }
\item \bf{How - Hard to objectively measure, but we will compare how much harder/easier we 
 fell it is to understand the mpi code. }
\end{itemize}
\item How easy it is to extend the program
\begin{itemize}
\item \bf{Why - If the application becomes harder to extend and improve upon 
when using mpi it maybe isn't feasible to write applications thats
needs to be extended further on with mpi.}
\item \bf{How - Measure how much work it takes to extend the application with 
		some example feature}
\end{itemize}
\item Performance - How fast do the application run
\begin{itemize}
\item \bf{Why - Because one of the main reason for scalability is to increase the
 performance we want to know how much we are able to increase it by using mpi. }
\item \bf{How - Measure the time to run the program with \emph{time}}
\end{itemize}
\item Scalability - how well do it scale when adding more resources
\begin{itemize}
\item \bf{Why - Optimal we would like to have an application that scales linear
...
\item \bf{How - Adding more resource to the system and then measure performance
 again}
\end{itemize}
\item Performance when using only one node, e.g. overhead for MPI
\begin{itemize}
\item \bf{Why - We want to know how much performance penalty we get for using
...
\item \bf{How - Setting up mpi with just one node and measure the performance
